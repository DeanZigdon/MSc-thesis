% table 5- pup presence. filtering best ADULT GLM for BINOMAL variables
all_seasons_data_adults = readtable("all_seasons_data_adults - Copy.csv");
var_lists = {{'density_5km','density_15km','date','parasites','FA_mom','season','temprature_IMS'},}; % density_5km, density_15km, season, date, parasites, preg_ratio , FA_mom , Weight_mom,pup
response_variable = 'pup'; % change if needed
significance_threshold = 0.05; % change if needed
...
all_seasons_data_adults = all_seasons_data_adults(all_seasons_data_adults.group == 1, :);
...
for group_idx = 1:length(var_lists)
    ...
    for r = 2:length(var_list)
        ...
        for i = 1:num_combinations
            ...
            if any(p_values > 0.055)
                continue;
            end
            log_likelihood = -0.5 * glme.LogLikelihood;
            num_params = length(glme.Coefficients.Estimate);
            aic_value = 2 * num_params - 2 * log_likelihood;
            numeric_predictor_variables = {'density_5km','density_15km','date','parasites','FA_mom','season','temprature_IMS'}; % orderding output of effect size #
