% table 8- GLM analysis of pup weight  2022-2023 +herz 22
all_seasons_pups_glm = readtable ("all_seasons_pups_glm.csv")

var_lists = {{'density_5km','density_15km','date','parasites','temprature_IMS','pup_gender'},};
response_variable = 'pup_weight';
    ...
    for r = 2:length(var_list)
        ...
        for i = 1:num_combinations
            ...
            try
                ...
                if any(p_values > 0.056) % change if needed
                    continue; 
                end
                log_likelihood = -0.5 * glme.LogLikelihood;
                num_params = length(glme.Coefficients.Estimate);
                aic_value = 2 * num_params - 2 * log_likelihood;
                numeric_predictor_variables = {'density_5km','density_15km','date','parasites','temprature_IMS','pup_gender'}; % this is the ordering of effect size # in the output.
