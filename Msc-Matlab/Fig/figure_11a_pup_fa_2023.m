%  figure 11a 2023 pup FA between colony types, boxplot 
...
fa_pup_rural = rural_data.FA_pup;
fa_pup_city = city_data.FA_pup;

figure;
subplot(1, 1, 1);
fa_pup_combined = [fa_pup_rural; fa_pup_city];
group_labels = [repmat({'Rural'}, size(fa_pup_rural, 1), 1); repmat({'City'}, size(fa_pup_city, 1), 1)];
h = boxplot(fa_pup_combined, group_labels, 'Whisker', 3.5, 'Symbol', 'o');
colors = ['g', 'r'];
for i = 1:2
    set(h(:, i), 'Color', colors(i));  
    set(h(1, i), 'MarkerEdgeColor', colors(i));
end
ylabel('Pup forearm [mm]', 'FontSize', 12, 'FontWeight', 'bold');
...
for i = 1:numel(unique_colonies_rural)
    ...
    colony_fa_pup = colony_data.FA_pup;
...
subplot(1, 1, 1); % returns to existing subplot
hold on;
x_positions = [1, 2];
scatter(x_positions(1) * ones(size(colony_fa_pup_values_rural)), colony_fa_pup_values_rural, 50, 'g', 's', 'LineWidth', 1.5);% disp mean value of rural (green)
scatter(x_positions(2) * ones(size(colony_fa_pup_values_city)), colony_fa_pup_values_city, 50, 'r', 's', 'LineWidth', 1.5);% disp mean value of urban (red)
for i = 1:numel(unique_colonies_rural)
